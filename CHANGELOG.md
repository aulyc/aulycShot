...
All notable changes to **aulycShot** will be documented in this file.
...
- Prepare the updater and public documentation for GitHub source Releases and a release-only Gitee mirror while retaining legacy mirror compatibility during migration
...
- Add an enabled-by-default automatic update check setting that can disable the daily silent check while keeping manual checks available

## [1.8.5] - 2026-08-01

### Changed

- Expand the About acknowledgements to include the age of AI, Codex, Claude, and aulyc while retaining thanks to the community, users, and open-source ecosystem
...
- Stabilize hosted macOS CI by separating interactive WindowServer tooltip checks from non-interactive verification
...
- Adopt Developer ID signing, notarization, release provenance, and manual private GitHub Releases without Homebrew dispatch
...
- Remove the legacy automatic GitHub release and Homebrew distribution workflow

Historical entries below were imported with the upstream MIT-licensed source archive and are retained as provenance for the inherited implementation.
...
### Changed
- Add README for aulycShot agent tools (8a00f33)
...
- Allow window detection to select aulycShot's own content windows (935902d)
