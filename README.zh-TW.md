...
<p align="center">
  macOS 上最順手的選單列截圖工具：雙擊 <code>⌘</code> 即刻截圖、標注、長截圖、美化和釘圖。
</p>
...
**macOS 上最順手的截圖工具。** 雙擊 `⌘` 即開即用——視窗一點就貼邊、選取範圍像素級精準、長截圖邊捲邊拼，再用浮動編輯器一氣呵成完成標注與美化。常駐選單列，不佔 Dock，無遙測、無訂閱、零第三方依賴。
...
- **直接編輯已有圖片**：在 Finder（桌面或任意視窗）中選取一張圖片檔案，再觸發截圖快速鍵，aulycShot 會跳過截圖，直接把這張圖載入標注編輯器。原始檔案不會被修改，編輯後的結果像一般截圖一樣進入剪貼簿和歷程記錄。
...
首次啟動時，aulycShot 會開啟設定視窗並顯示兩項權限狀態。權限都授予後即可啟動應用程式。
...
選單列 **歷程** 子選單會把最近截圖和取色記錄儲存在 `~/Library/Application Support/aulycShot/History`。點擊圖片項目會重新複製該截圖，點擊顏色項目會複製對應色值，也可以從子選單清除全部歷程。

## 專案結構

- `aulycShot/App/`：應用程式進入點、AppDelegate 和套件中繼資料
- `aulycShot/Capture/`：截圖遮罩、選取範圍、視窗偵測、ScreenCaptureKit 擷取、長截圖拼接、剪貼簿和歷程記錄
- `aulycShot/Editor/`：標注模型、編輯畫布、浮動工具列、美化渲染、馬賽克、長截圖預覽和釘圖視窗
- `aulycShot/OCR/`：文字辨識服務、結果面板和辨識區域選取
