...
# complete package instead of incorrectly reporting that nothing changed.
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
  ...
  echo "No Git metadata found — checking the complete Swift package"
  changed_files=$( {
      printf '%s\n' Package.swift
      find aulycShot aulycShot-share-extension Tests -type f -name '*.swift' -print
    } | sort -u)
fi
