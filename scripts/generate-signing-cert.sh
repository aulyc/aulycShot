#!/usr/bin/env bash
#
# Generate a self-signed code-signing certificate for aulycShot.
#
# Why this exists:
...
# Env:
#   CERT_PASSWORD   password protecting the .p12 (default: aulycShot)
#   CERT_NAME       certificate common name (default: aulycShot Self-Signed)
#   CERT_DAYS       validity in days (default: 3650 — ~10 years)
#
set -euo pipefail

OUT_DIR="${1:-$HOME/Desktop}"
CERT_NAME="${CERT_NAME:-aulycShot Self-Signed}"
CERT_PASSWORD="${CERT_PASSWORD:-aulycShot}"
...
P12="$OUT_DIR/aulycShot-signing.p12"
...
B64="$OUT_DIR/aulycShot-signing.p12.base64"
base64 -i "$P12" -o "$B64"
