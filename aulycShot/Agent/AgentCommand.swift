...
enum AgentCommand {
    static func runIfRequested(arguments: [String]) -> Int32? {
        ...
        do {
            ...
            AgentIO.writeStderr("aulycShot agent error: \(error.message)")
            return error.exitCode
        } catch {
            AgentIO.writeStderr("aulycShot agent error: \(error.localizedDescription)")
            return 1
        }
    }
}
...
private struct AgentCLI {
    ...
    Usage
      aulycShot agent annotate --input image.png --spec marks.json --out result.png
      aulycShot agent capture --target mouse-screen --out shot.png
      aulycShot agent run --target mouse-screen --spec marks.json --out result.png
