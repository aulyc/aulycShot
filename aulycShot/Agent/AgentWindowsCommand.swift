...
struct AgentWindowsOptions {
    ...
    Usage
      ...
      aulycShot agent list-windows --owner Safari --limit 5 --pretty
    ...
    static func list(options: AgentWindowsOptions) -> AgentWindowsResult {
        ...
        let metadata: [String: Any] = [
            ...
            "windows": limited.map { window in
                ...
                item["captureCommand"] = "aulycShot agent capture --target window-id --window-id \(window.windowID) --out shot.png"
                return item
            }
        ]
