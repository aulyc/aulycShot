...
final class HistoryTextContent {
    ...
    private static let loadQueue = DispatchQueue(
        label: "aulycShot.historyTextContent",
        qos: .utility,
...
final class HistoryManager {
    ...
    private let queue = DispatchQueue(label: "aulycShot.history", qos: .utility)
    ...
    private init() {
        ...
        let historyDirectoryURL = base.appendingPathComponent("aulycShot/History", isDirectory: true)
    ...
    private func entries(in directory: URL, allowedExtensions: Set<String>) -> [HistoryEntry] {
        ...
        return urls.compactMap { url in
            ...
            case "mp4":
                return HistoryEntry(fileURL: url, createdAt: date, kind: .video)
            case "color":
