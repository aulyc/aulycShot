...
/// Locates and reads the most recent aulycShot diagnostic log. This includes the
...
enum CrashLogReader {
    ...
    /// name matches aulycShot's CFBundleExecutable.
    private static let processName = "aulycShot"

    /// Returns the newest app diagnostic log or macOS crash report belonging
    /// to aulycShot, or nil if no log exists. This only stats the files — it does
    ...
    /// crash report belonging to aulycShot, or nil if the app has never crashed.
    ...
    private static func latestCrashFile() -> Entry? {
        ...
        for dir in diagnosticReportDirs() {
            ...
            for url in entries {
                guard isAulycShotCrashReport(url) else { continue }
    ...
    /// Deletes aulycShot's persisted diagnostic log and any aulycShot-owned macOS
    ...
    static func deleteAllLogs() {
        ...
        for dir in diagnosticReportDirs() {
            ...
            for url in entries where isAulycShotCrashReport(url) {
    ...
    private static func isAulycShotCrashReport(_ url: URL) -> Bool {
