...
<p align="center">
  Инструмент скриншотов в строке меню macOS: дважды нажмите <code>⌘</code>, чтобы сделать снимок, разметить его, собрать длинную страницу или закрепить.
</p>
...
- **Закрепление снимков**: готовое изображение можно держать поверх остальных окон для справки.
...
Единый статус функций становится доступным только после включения разрешений Accessibility и Screen Recording. В противном случае снимки и запись блокируются, а приложение показывает объединённую инструкцию по разрешениям.
...
3. Используйте плавающую панель для разметки, прокрутки, сохранения, закрепления или подтверждения.
...
| Прокрутка / закрепление | Финальная обработка и обмен |

## Настройки

В настройках можно выбрать язык, значок строки меню, запуск при входе, демо-режим, шорткаты и быстрые переходы к системным разрешениям. Интерфейс поддерживает 简体中文 и English.
