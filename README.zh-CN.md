...
<p align="center">
  macOS 上最顺手的菜单栏截图工具：双击 <code>⌘</code> 即刻截图、标注和钉图。
</p>
...
**macOS 上最顺手的截图工具。** 双击 `⌘` 即开即用——窗口一点就贴边、选区像素级精准，再用浮动编辑器一气呵成完成标注。常驻菜单栏，不占 Dock，无遥测、无订阅、零第三方依赖。
...
- `aulycShot/Capture/`：截图遮罩、选区、窗口检测、ScreenCaptureKit 捕获和剪贴板
- `aulycShot/Editor/`：标注模型、编辑画布、浮动工具栏、马赛克和钉图窗口
...
3. 感谢开源项目 [capcap](https://github.com/realskyrin/capcap)，为 aulycShot 提供最初的灵感与基础
