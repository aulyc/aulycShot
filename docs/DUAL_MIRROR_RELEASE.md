...
安装包、更新元数据和指向 GitHub 源码的说明，不得向 Gitee 推送源码。

目标中央渠道映射使用上述同名 GitHub/Gitee 仓库。GitHub 说明为简体中文在前、
English 在后，Gitee 仅简体中文。该映射使用 `macos-compact` /
`dual-manifest`：GitHub 和 Gitee Release 都只允许同一份已验证 DMG；DMG checksum
和 provenance checksum 只作为本地验证证据；最终 provenance 按
`updates/<version>/<file>` 写入两端更新渠道分支，`latest.json` 使用 Schema v2
指向两端 DMG 与版本化 provenance。构建目录和其他本地验证证据不会成为公开附件。

应用内更新器同时识别旧 Schema v1 和当前 Schema v2，优先读取 GitHub manifest，
失败后读取 Gitee。无论来源均验证
...
`1.8.10` 已完成公开渠道迁移，`1.8.11` 已移除旧兼容仓库。`1.8.12` 是
`macos-compact` 与 Schema v2 的迁移版本；因为 `1.8.11` 只识别 Schema v1，
这次需从 Release 手动更新，之后继续使用应用内自动更新。
...
写远端。发布顺序为两端 DMG Release、Gitee 版本化 provenance 与 manifest、GitHub
版本化 provenance 与 manifest，最后独立回读。任一端失败都保留无凭据状态并只
允许同计划向前重试；禁止覆盖旧版本或追加额外 Release 附件。
